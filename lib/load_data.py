...
def get_info(name, root, mode='rgb', split=1):
    ...
    data_dir = '/home/zoro/AI/action_recognition/I3D_Finetune/data/'+name+'/'+mode+'.txt'
    test_split = '/home/zoro/AI/action_recognition/I3D_Finetune/data/'+name+'/testlist%02d' % split+'.txt'
    f_1 = open(data_dir, 'r')
    ...
    for line in f_1.readlines():
        ...
        if line[0] in test:
            ...
            test_have.append(line[0])
        else:
            train_info.append(info)
    print(len(test_info))
    print(len(train_info))
    ret_list = list(set(test) ^ set(test_have))
    print(len(ret_list))
    f_1.close()
...
class UCF101():
    ...
    def get_info(root, mode='rgb', split=1):
        ...
        test_split = 'data/ucf101/testlist%02d' % split+'.txt'
        f_1 = open(data_dir, 'r')
        test = [x.split('/')[1].split('.')[0]
                for x in open(test_split, 'r').readlines()]
        train_info = []
